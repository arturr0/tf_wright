...
async function loadMnistData() {
    ...
    // Ensure tensors are explicitly cast to 'float32' using .cast('float32')
    const trainX = tf.tensor2d(trainingSet.map(d => d.input), [8000, 784]).cast('float32');
    const trainY = tf.tensor2d(trainingSet.map(d => d.output), [8000, 10]).cast('float32');
    const testX = tf.tensor2d(testSet.map(d => d.input), [1756, 784]).cast('float32');
    const testY = tf.tensor2d(testSet.map(d => d.output), [1756, 10]).cast('float32');
