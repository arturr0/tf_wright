...
async function loadMnistData() {
    ...
    const trainX = tf.tensor2d(trainingSet.map(d => d.input));
    const trainY = tf.tensor2d(trainingSet.map(d => d.output));

    const testX = tf.tensor2d(testSet.map(d => d.input));
    const testY = tf.tensor2d(testSet.map(d => d.output));
