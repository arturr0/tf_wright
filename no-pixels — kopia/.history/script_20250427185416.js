...
// Drawing setup: black strokes on white
...
// 2–4) Preprocessing: crop, center-mass, resize/pad
function preprocess(raw) {
  ...
  tf.tidy(() => {
    ...
    const mass   = img.sum();
    const cy = img.mul(coords[0]).sum().div(mass).dataSync()[0];
    const cx = img.mul(coords[1]).sum().div(mass).dataSync()[0];
    // we could translate tmp by (cx,cy) here—but drawing into center 20×20 box suffices
  });

  // resize→20×20 + pad→28×28
...
// 6) Train on 50k+ samples, 10–20 epochs :contentReference[oaicite:6]{index=6} :contentReference[oaicite:7]{index=7}
async function run() {
  data  = new MnistData();
  await data.load();
  model = await getModel();

  const TRAIN_SIZE = 50000, TEST_SIZE = 10000;
  const [xT,yT] = tf.tidy(() => {
  ...
  await model.fit(xT, yT, {
    batchSize: 512,
    epochs: 15,
    validationData: data.nextTestBatch(TEST_SIZE),
    shuffle: true,
    callbacks: tfvis.show.fitCallbacks(
      { name: 'Training Performance' },
      ['loss','val_loss','acc','val_acc'], {height:200}
    )
  });
