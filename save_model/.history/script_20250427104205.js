...
canvas.addEventListener('mouseup', () => {
  isDrawing = false;
  ctx.beginPath(); // reset path to avoid weird connections
});
...
function draw(e) {
  if (!isDrawing) return;
  ctx.lineWidth = 2; // thinner line, MNIST-like
  ctx.lineCap = 'round';
  ctx.strokeStyle = 'black';

  // Scale mouse coordinates because of display-size difference
...
async function predictCanvas() {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    // Since background is white (255) and drawing is black (0), 
    // we want black to be 1.0 and white to be 0.0
    grayData[i / 4] = 1.0 - (avg / 255); 
  }
